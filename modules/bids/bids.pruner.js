...
let BidsPruner = BidsPruner_1 = class BidsPruner {
    constructor(drizzle, ordClient, bidsService) {
    ...
    async dropRow(row, live) {
        await this.bidsService.deleteByOutpointAndBuyer(row.network, row.catTxid, row.catVout, row.buyerOrdinalsAddress);
        this.logger.log(`Prune: dropped bid on ${row.catTxid}:${row.catVout} from buyer ${row.buyerOrdinalsAddress} — ` +
