...
describe('ListingsPruner.runPrune', () => {
    it('is a no-op when the table is empty', async () => {
        ...
        const pruner = new listings_pruner_1.ListingsPruner(drizzle, ord, listings);
    ...
    it('processes multiple listings independently — ord flake on one does not abort the rest', async () => {
        ...
        const ord = {
            getCatCurrentLocation: jest.fn()
                .mockResolvedValueOnce({ txid: 'aa'.repeat(32), vout: 0, ordinalsAddress: ORD_ADDR })
                .mockRejectedValueOnce(new Error('flake'))
                .mockResolvedValueOnce({ txid: OTHER_TXID, vout: 0, ordinalsAddress: ORD_ADDR }),
        };
        const listings = { deleteByIdIfUnchanged: jest.fn().mockResolvedValue(undefined) };
        const pruner = new listings_pruner_1.ListingsPruner(drizzle, ord, listings);
        await pruner.runPrune();
...
//# sourceMappingURL=listings.pruner.spec.js.map