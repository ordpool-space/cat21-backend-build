...
describe('CatsController', () => {
    ...
    beforeEach(() => {
        ...
        controller = new cats_controller_1.CatsController(service);
    });
    describe('getHealth', () => {
    ...
    describe('getCatByNumber', () => {
        ...
        it('should throw NotFoundException for unknown cat', async () => {
            ...
            const reply = createMockReply();
            await expect(controller.getCatByNumber(999999, reply)).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('getCatByTxHash', () => {
        it('should return a cat for valid 64-char hex hash', async () => {
            service.getCatByTxHash.mockResolvedValue(mockCat);
            const reply = createMockReply();
        ...
        it('should throw NotFoundException for unknown tx hash', async () => {
            ...
            const reply = createMockReply();
            await expect(controller.getCatByTxHash(mockCat.txHash, reply)).rejects.toThrow(common_1.NotFoundException);
        });
        it('should reject invalid tx hash (too short)', async () => {
            const reply = createMockReply();
            await expect(controller.getCatByTxHash('abc123', reply)).rejects.toThrow(common_1.NotFoundException);
            expect(service.getCatByTxHash).not.toHaveBeenCalled();
        });
        it('should reject invalid tx hash (uppercase)', async () => {
            const reply = createMockReply();
            await expect(controller.getCatByTxHash(mockCat.txHash.toUpperCase(), reply)).rejects.toThrow(common_1.NotFoundException);
            expect(service.getCatByTxHash).not.toHaveBeenCalled();
        });
        it('should reject invalid tx hash (non-hex characters)', async () => {
            const reply = createMockReply();
            const badHash = 'z'.repeat(64);
            await expect(controller.getCatByTxHash(badHash, reply)).rejects.toThrow(common_1.NotFoundException);
            expect(service.getCatByTxHash).not.toHaveBeenCalled();
        });
    });
    describe('getCats', () => {
        ...
        it('should cap itemsPerPage at 100', async () => {
            service.getCats.mockResolvedValue(emptyPage);
    ...
    describe('getCatSvg', () => {
        it('should throw NotFoundException for unknown cat', async () => {
            ...
            const reply = createMockReply();
            await expect(controller.getCatSvg(999999, reply)).rejects.toThrow(common_1.NotFoundException);
        });
        it('should send SVG with correct headers', async () => {
            service.getCatSvg.mockResolvedValue('<svg>test</svg>');
            const reply = createMockReply();
    ...
    describe('getCatWebp', () => {
        it('should throw NotFoundException for unknown cat', async () => {
            service.getCatSvg.mockResolvedValue(null);
            const reply = createMockReply();
            await expect(controller.getCatWebp(999999, reply)).rejects.toThrow(common_1.NotFoundException);
        });
        it('should send WebP with correct headers for valid cat', async () => {
            service.getCatSvg.mockResolvedValue('<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22"><rect width="22" height="22" fill="red"/></svg>');
            const reply = createMockReply();
            await controller.getCatWebp(0, reply);
            expect(reply.header).toHaveBeenCalledWith('Content-Type', 'image/webp');
            expect(reply.header).toHaveBeenCalledWith('Content-Disposition', 'inline; filename="cat21-0.webp"');
...
describe('CatsController — extended health, image endpoints, pagination clamps', () => {
    const VALID_SVG = '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"><rect width="20" height="20" fill="#ff9900"/></svg>';
    function make(svc = {}) {
        const service = {
        ...
        return { controller: new cats_controller_1.CatsController(service), service };
    }
    describe('getExtendedHealth', () => {
        ...
        it('throws 503 ServiceUnavailable (carrying the report) when status is down', async () => {
            const health = { status: 'down', db: { reachable: false } };
            const { controller } = make({ getExtendedHealth: jest.fn().mockResolvedValue(health) });
            const reply = createMockReply();
            const { ServiceUnavailableException } = await Promise.resolve().then(() => require('@nestjs/common'));
            let thrown;
            try {
                await controller.getExtendedHealth(reply);
            }
            catch (e) {
                thrown = e;
            }
            expect(thrown).toBeInstanceOf(ServiceUnavailableException);
            expect(thrown.getResponse()).toMatchObject({ status: 'down' });
        });
    });
    describe('getCatSvg', () => {
        ...
        it('404s with no-store when the cat has no svg', async () => {
            ...
            const reply = createMockReply();
            await expect(controller.getCatSvg(999, reply)).rejects.toBeInstanceOf(common_1.NotFoundException);
            expect(reply.header).toHaveBeenCalledWith('Cache-Control', 'no-store');
        });
    });
    describe('getCatWebp', () => {
        ...
        it('404s with no-store when the cat has no svg', async () => {
            const { controller } = make({ getCatSvg: jest.fn().mockResolvedValue(null) });
            const reply = createMockReply();
            await expect(controller.getCatWebp(999, reply)).rejects.toBeInstanceOf(common_1.NotFoundException);
            expect(reply.header).toHaveBeenCalledWith('Cache-Control', 'no-store');
        });
        it('500s with no-store when the svg cannot be rendered', async () => {
            ...
            const reply = createMockReply();
            await expect(controller.getCatWebp(0, reply)).rejects.toBeInstanceOf(common_1.InternalServerErrorException);
            expect(reply.header).toHaveBeenCalledWith('Cache-Control', 'no-store');
        });
    });
    describe('getCatSocialCard', () => {
        ...
        it('404s with no-store when the cat is missing', async () => {
            ...
            const reply = createMockReply();
            await expect(controller.getCatSocialCard(999, reply)).rejects.toBeInstanceOf(common_1.NotFoundException);
            expect(reply.header).toHaveBeenCalledWith('Cache-Control', 'no-store');
        });
        it('500s with no-store when the art cannot be rendered', async () => {
            ...
            const reply = createMockReply();
            await expect(controller.getCatSocialCard(0, reply)).rejects.toBeInstanceOf(common_1.InternalServerErrorException);
            expect(reply.header).toHaveBeenCalledWith('Cache-Control', 'no-store');
        });
    });
